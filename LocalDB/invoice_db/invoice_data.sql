...
/*
sex::sex_type is the syntax for type casting sex which is a char data type
to sex_type which is an enum data type.
*/
...
cc_expiry_month SMALLINT NOT NULL,
cc_expiry_day SMALLINT NOT NULL,
cc_secret_code SMALLINT NOT NULL,
...
CREATE INDEX transaction_index ON transaction(name);


-- Create Multi Column Index --
CREATE INDEX transaction_index2 ON transaction(name, payment_type);


-- Delete all the data in a table - Truncate Table --
...
ALTER TABLE customer ALTER COLUMN zip TYPE INTEGER USING zip::INTEGER;
...
-- Change Column Data Type --
ALTER TABLE sales_order ALTER COLUMN order_number TYPE BIGINT;


INSERT INTO sales_order(customer_id, sales_person_id, order_time, order_number, cc_number, cc_expiry_month, cc_expiry_day, cc_secret_code, cc_name) VALUES
...
INSERT INTO sales_item(item_id, sales_order_id, quantity, discount, taxable, sales_tax_rate) VALUES
...
-- Find all sales item where discount > 15%
SELECT *
FROM sales_item
WHERE discount > 0.15;


-- Find all order times in december 2018
SELECT order_time
FROM sales_order
WHERE order_time >= '2018-12-01' AND order_time <= '2018-12-31';
...
-- Find top 5 sales item where discount > 15%
SELECT *
FROM sales_item
WHERE discount > 0.15
...
SELECT *
FROM customer
WHERE state IN ('CA', 'GA', 'IL');


-- Joins --
-- Find all the items that were ever sold
...
-- Join using WHERE (not recommended) it is old style of joining tables
SELECT item.id, item.price, sales_item.quantity, sales_item.discount
...
-- Left Join --
SELECT product.id, product.name, product.supplier, item.price
FROM product
LEFT JOIN item
ON product.id = item.product_id;


-- Cross Join or Cartesian Product of both tables --
SELECT sales_item.sales_order_id, sales_item.quantity, item.product_id
FROM sales_item
CROSS JOIN item
ORDER BY sales_item.sales_order_id;


--- Union --
/*
Union combines the result of two or more select statements into one.
The number and order of columns for both the queries must be same and their respective
data types should be compatible.
*/
-- Find info about all customers and sales person born in the month of december
...
-- Find all customers whose first name starts with 'A' and has exactly 5 characters after it
SELECT first_name, last_name
FROM customer
WHERE first_name LIKE 'A_____';


-- Find all customers whose first name starts with D or last name ends with 'n'
SELECT first_name, last_name
FROM customer
WHERE first_name SIMILAR TO 'D%' OR last_name SIMILAR TO '%n';


-- Find all customers whose first name starts with 'Ma'
SELECT first_name, last_name
FROM customer
...
-- Find all customers whose last name ends with 'ez'
SELECT first_name, last_name
FROM customer
WHERE last_name ~ 'ez$';


-- Find all customers whose last name contains 'ez' or 'nd'
SELECT first_name, last_name
FROM customer
WHERE last_name ~ 'ez|nd';


-- Find all customers whose last name contains any letters from w to z
SELECT first_name, last_name
FROM customer
WHERE last_name ~ '[w-z]';
...
-- Group By --
-- Group By defines how records are grouped and returns a single row for each unique value
-- Find months and number of customers having birthdays in those months
...
-- Having --
-- Having can be used with group by to apply a filter to the results returned by group by
-- Find months where more than one customer have birthdays
SELECT EXTRACT(MONTH FROM birth_date) AS month, COUNT(*)
FROM customer
GROUP BY month
HAVING COUNT(*) > 1
...
-- Find total price of all the items
SELECT SUM(price)
FROM item;


-- Find total number of items and the total, average, min and max price of all the items
...
-- Views --
/* Views are virtual tables stored in the database which are created from the 
result of a select query. Simple views do not physically store any data. */
...
JOIN sales_person
ON sales_order.sales_person_id = sales_person.id
...
-- Updates will not work as the view contains joins
-- UPDATE purchase_order_overview SET product_name = 'Dom' WHERE order_number = 20166617;
...
SELECT fn_add_ints(2, 10);


-- Function using dollar $ quotes
CREATE OR REPLACE FUNCTION fn_add_ints(int, int) RETURNS int AS
$body$
	SELECT $1 + $2;
$body$
...
-- Write a function to set state to 'CA' for all customers whose company is 'BP'
CREATE OR REPLACE FUNCTION update_emp_state() RETURNS void AS
$body$
	UPDATE customer
	SET state = 'CA'
	WHERE company = 'BP';
...
-- Write a function to find the max price of all the items
CREATE OR REPLACE FUNCTION fn_find_max_price() RETURNS numeric AS
$body$
	SELECT MAX(price)
	FROM item;
$body$
...
SELECT fn_find_max_price();


-- Write a function to find the total value of inventory
CREATE OR REPLACE FUNCTION fn_find_inventory_value() RETURNS numeric AS
$body$
	SELECT SUM(price)
	FROM item;
$body$
...
SELECT fn_find_inventory_value();


-- Write a function to calcuate the total number of customers
CREATE OR REPLACE FUNCTION fn_customers_count() RETURNS int AS
$body$
	SELECT COUNT(*)
	FROM customer;
$body$
...
SELECT fn_customers_count();


-- Write a function to find the number of customers who have no phone
CREATE OR REPLACE FUNCTION fn_customers_count_no_phone() RETURNS int AS
$body$
	SELECT COUNT(*)
	FROM customer
	WHERE phone IS NULL;
...
SELECT fn_customers_count_no_phone();


-- Named Parameters in Functions --
-- Write a function to find the count of customers from a state
CREATE OR REPLACE FUNCTION fn_customers_count_from_state(state_name char(2)) RETURNS int AS
$body$
	SELECT COUNT(*)
	FROM customer
...
SELECT fn_customers_count_from_state('TX');


-- Write a function to get the number of orders from a specific customer
CREATE OR REPLACE FUNCTION fn_get_orders_count_for_customer(cust_fname varchar(30), cust_lname varchar(30)) RETURNS int AS
$body$
	SELECT count(*)
	FROM sales_order
	JOIN customer
	ON sales_order.customer_id = customer.id
	WHERE customer.first_name = cust_fname AND customer.last_name = cust_lname;
$body$
LANGUAGE SQL


SELECT fn_get_orders_count_for_customer('Michael', 'Jackson');
...
JOIN customer
ON sales_order.customer_id = customer.id
WHERE customer.first_name = 'Michael' AND customer.last_name = 'Jackson';


-- Returning a row / composite
-- Write a function to return the last order
...
-- Returns the entire rows
SELECT fn_get_last_order();


-- Returns whole data as a table format
SELECT (fn_get_last_order()).*;


-- Returns any particular column
SELECT (fn_get_last_order()).order_time;


-- Use SETOF to return multiple rows
-- Write a function to return details of all employees from a state
CREATE OR REPLACE FUNCTION fn_get_employees_by_location(state char(2)) RETURNS SETOF sales_person AS
$body$
	SELECT *
	FROM sales_person
	WHERE state = state;
...
-- Get first_name, last_name and phone of employees using the above function
SELECT first_name, last_name, phone
FROM fn_get_employees_by_location('CA');


-- Function using pl/pgSQL
-- Write a function to get product price by product name
CREATE OR REPLACE FUNCTION fn_multiply_ints(int, int) RETURNS int AS
$body$
BEGIN
	RETURN $1 * $2;
END;
...
SELECT fn_multiply_ints(4, 3);


-- Write a function to find the highest price of a product by its name
CREATE OR REPLACE FUNCTION fn_get_price_by_product_name(product_name varchar(30)) RETURNS numeric AS
$body$
BEGIN
	RETURN item.price
	FROM item
...
SELECT fn_get_price_by_product_name('Grandview');
...
-- Use SQL query to perform assignment
-- Write a function to find a random number in a range
CREATE OR REPLACE FUNCTION fn_find_random_num(min_val int, max_val int) RETURNS int AS
$body$
DECLARE
	rand int;
BEGIN
	SELECT RANDOM() * (max_val - min_val) + min_val
	INTO rand;
	RETURN rand;
END;
$body$
...
SELECT fn_find_random_num(1, 10);


-- Write a function to get a random sales person name
-- Use RECORD type to store a row
CREATE OR REPLACE FUNCTION fn_get_random_sales_person() RETURNS varchar AS
$body$
DECLARE
	rand int;
	total_sales_person_count int;
	random_sales_person record;
BEGIN
	SELECT COUNT(*) 
	FROM sales_person 
	INTO total_sales_person_count;
	
	SELECT RANDOM() * (total_sales_person_count - 1) + 1 
	INTO rand;
	
	SELECT * 
	FROM sales_person 
	WHERE id = rand 
	INTO random_sales_person;

	RETURN CONCAT(random_sales_person.first_name, ' ', random_sales_person.last_name);
END;
$body$
LANGUAGE plpgsql


SELECT fn_get_random_sales_person();


-- Use IN/OUT instead of RETURNS
-- Write a function to calculate the sum of 3 numbers
CREATE OR REPLACE FUNCTION fn_get_3sum(IN val1 int, IN val2 int, IN val3 int, OUT res int) AS
$body$
BEGIN
	res := val1 + val2 +val3;
END;
...
SELECT fn_get_3sum(1, 2, 3);


-- Using multiple IN and OUT
-- Write a function to get birthday of a customer whose birth month is given
CREATE OR REPLACE FUNCTION fn_get_customer_birthday(
	IN in_month int,
	OUT cust_birth_day int,
	OUT cust_birth_month int,
	OUT cust_first_name varchar,
	OUT cust_last_name varchar
) AS
$body$
BEGIN
	SELECT 
		...
		last_name
	FROM customer
	WHERE EXTRACT(MONTH FROM birth_date) = in_month
	LIMIT 1
	INTO cust_birth_day, cust_birth_month, cust_first_name, cust_last_name;
END;
$body$
LANGUAGE plpgsql


SELECT fn_get_customer_birthday(3);


-- Return query results using QUERY
-- Write a function to get all the salespersons
CREATE OR REPLACE FUNCTION fn_get_sales_persons() RETURNS SETOF sales_person AS
$body$
BEGIN
	RETURN QUERY
	SELECT *
...
-- Return any specific column
SELECT (fn_get_sales_persons()).state;


-- Return data from multiple tables using TABLE
-- Write a function to find top 10 expensive products
...
CREATE OR REPLACE FUNCTION fn_get_top_10_expensive_products()
RETURNS TABLE (
	name varchar(30),
	supplier VARCHAR(30),
	price numeric(6, 2)
...
-- Returns entire rows
SELECT fn_get_top_10_expensive_products();
...
-- Use IF ELSE
-- Write a function to check the number of orders in a given month and provide output as good/bad/fine
CREATE OR REPLACE FUNCTION fn_check_company_performance(order_month int) RETURNS varchar AS
$body$
DECLARE
	orders_in_month int;
BEGIN
	...
	FROM sales_order
	WHERE EXTRACT(MONTH FROM order_time) = order_month
	INTO orders_in_month;

	IF orders_in_month > 5 THEN
		RETURN CONCAT(orders_in_month, ' orders - Doing Good!');
	ELSIF orders_in_month < 2 THEN
		RETURN CONCAT(orders_in_month, ' orders - Doing Bad!');
	ELSE 
		RETURN CONCAT(orders_in_month, ' orders - Doing Fine!');
	END IF;
END;
$body$
LANGUAGE plpgsql
...
SELECT fn_check_company_performance(7);


-- Use case statements
-- Write a function to check the number of orders in a given month and provide output as good/bad/fine
CREATE OR REPLACE FUNCTION fn_check_monthly_performance(order_month int) RETURNS varchar AS
$body$
DECLARE
	orders_in_month int;
BEGIN
	...
	CASE
		WHEN orders_in_month > 5 THEN
			RETURN CONCAT(orders_in_month, ' orders - Doing Good!');
		WHEN orders_in_month < 2 THEN
			RETURN CONCAT(orders_in_month, ' orders - Doing Bad!');
		ELSE
			RETURN CONCAT(orders_in_month, ' orders - Doing Fine!');
	END CASE;
...
SELECT fn_check_monthly_performance(4);


-- Using LOOP
-- Write a function to calculate sum of numbers from 1 to N using LOOP
CREATE OR REPLACE FUNCTION fn_test_loop(n int) RETURNS int AS
$body$
DECLARE
	i int DEFAULT 1;
...
Syntax:
FOR i IN 1 .. n BY 2
i = Loop variable which is automatically declared
n = Loop will run till i is less than n
BY 2 = Increments i by 2
*/
-- Use a for loop to calculate the sum of all odd numbers from 1 to N
CREATE OR REPLACE FUNCTION fn_test_for_loop(n int) RETURNS int AS
$body$
DECLARE
	total_sum int DEFAULT 0;
BEGIN
...
SELECT fn_test_for_loop(10);


-- Run the previous FOR LOOP in reverse
-- Use a for loop to calculate the sum of all odd numbers from N to 1
CREATE OR REPLACE FUNCTION fn_test_for_loop_2(n int) RETURNS int AS
$body$
DECLARE
	total_sum int DEFAULT 0;
BEGIN
	-- Needed as we calculating sum of 1 to N odd numbers
	IF n % 2 = 0 THEN
		n := n - 1;
	END IF;

	FOR i IN REVERSE n .. 1 BY 2
	LOOP
		total_sum := total_sum + i;
	END LOOP;
...
SELECT fn_test_for_loop_2(10);


-- Use a DO block to directly execute procedural code without creating a function
-- Use FOR loop to iterate records from an SQL QUERY Result and RAISE NOTICE to log the data to output
DO
$body$
DECLARE rec record;
BEGIN
...
-- Create an ARRAY and use a FOREACH loop to traverse it
DO
$body$
DECLARE
	arr int[] := ARRAY[22, 56, 91];
	i int;
BEGIN
...
-- Use WHILE Loop
-- Calculate sum of numbers from 1 to 50 and print it on the console
DO
$body$
DECLARE
	i INT DEFAULT 1;
...
-- Use CONTINUE to print only odd numbers from 1 to N
DO
$body$
DECLARE
 i int DEFAULT 0;
...
-- Write a function to calculate the total inventory value of a given supplier
CREATE OR REPLACE FUNCTION fn_get_supplier_value(sp_name varchar) RETURNS varchar AS
$body$
DECLARE
...
BEGIN
	...
	JOIN item
	ON product.id = item.product_id
	WHERE product.supplier = sp_name
	GROUP BY product.supplier
	INTO supplier_name, supplier_value;

	RETURN CONCAT(supplier_name, ' - Inventory Value: $', supplier_value);
END;
$body$
LANGUAGE plpgsql
...
-- Write a stored procedure to update balance in past_due table for a payment that was made
...
-- Return value from a procedure using INOUT parameter
/* INOUT parameter acts as both input + output. We pass an initial value, procedure modifies it
and returns an updated value. */
-- Write a procedure to increment a value by 10
CREATE OR REPLACE PROCEDURE increment_value(INOUT num int)
AS
$body$
BEGIN
	num := num + 10;
...
CALL increment_value(54);


-- Triggers
-- Create distributor table and a trigger to track changes made in the distributor table
...
/* Create distributor_audit table to track changes made to the distributor table 
...
BEGIN
	IF NEW.name <> OLD.name THEN
		INSERT INTO distributor_audit(distributor_id, name, edit_date)
		VALUES
		(OLD.id, OLD.name, NOW());
	END IF;
...
-- Update the table to execute the trigger
UPDATE distributor
SET name = 'Western Clothing'
WHERE id = 2;


-- Check the distributor_audit table
SELECT *
FROM distributor_audit;


-- Conditional Trigger
/* We can use conditional triggers to prevent update, delete on tables */
-- Write a trigger to block changes to the distributor table on weekend
-- First delete the previous trigger so that it does not intervene
DROP TRIGGER tr_dist_name_changed ON distributor;
